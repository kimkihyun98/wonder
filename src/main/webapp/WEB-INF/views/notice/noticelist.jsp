	...
	<div data-v-76574781="">
		...
		<div data-v-76574781="" class="content_area">
			...
			<c:forEach var="Notice" items="${list}">	
			 <c:if test="${Notice.notice_sort eq '공지사항'}">
			...
			<ul class="pagination">
				<c:if test="${pageMaker.prev}">
					<li class="list"><a href="notice?page=${pageMaker.startPage-1}">&laquo;</a></li>
				</c:if>
				<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
				 	<li class="list">
				 		<a 
				 		<c:out value="${pageMaker.cri.page == idx?'style=color:blue':''}"/>
				 		href="notice?page=${idx}">${idx}</a>
				    </li> 			
				</c:forEach>
				
				<c:if test ="${pageMaker.next && pageMaker.endPage >0 }">
					<li class="list"><a href ="notice?page=${pageMaker.endPage+1 }">&raquo;</a></li>
	   			</c:if>
